          ...
          allOf: [
            { $ref: "#/components/schemas/EnsembleConfiguration" },
            {
              type: "object", required: ["source"], properties: {
                ...
                source: {
                  oneOf: [
                    { type: "object", required: ["type", "symbol"], properties: { type: { const: "ticker" }, symbol: { type: "string" }, provider: { type: "string", enum: ["auto", "alpaca", "yahoo"] }, start: { type: "string", format: "date" }, frequency: {type:"string",enum:["1Day","1Hour","1Min"],default:"1Day"}, field: { type: "string", enum: ["open", "high", "low", "close", "volume"] } } },
                    { type: "object", additionalProperties: false, required: ["type", "provider", "symbol", "contract_id"], properties: { type: { const: "prediction_market" }, provider: { type: "string", enum: ["polymarket_us", "kalshi"] }, symbol: { type: "string", description: "Provider market slug (Polymarket US) or market ticker (Kalshi)." }, contract_id: { type: "string", description: "Exact selected side ID from market lookup; verified server-side." }, frequency: { type: "string", enum: ["1min", "1h", "1D"], default: "1min" }, history_phase: {type:"string",enum:["both","pregame","in_game"],default:"both",description:"Uses provider-confirmed event start. Unknown start rejects phase-specific requests."}, history_lookback_minutes:{type:"integer",minimum:0,maximum:129600,default:0,description:"0 selects up to 500 available bars; positive values restrict elapsed history before cutoff. For pregame, the window ends at the earlier of cutoff and game start. No missing bars are invented."} } },
