...
- Hold the filled side until authoritative settlement unless its executable economic bid reaches $0.05 or lower. A five-cent stop trigger is durable: while the market is active, the worker checks roughly once per second and submits a reduce-only IOC sell at the latest executable bid. The trigger remains latched if the bid later rebounds; each retry uses the latest bid for only the unsold quantity. It reconciles each order and its actual fills before retrying with a fresh client order ID. A rejected, unfilled, partially filled, or ambiguous order is never mistaken for a flat position. If the market settles before the stop fully fills, the residual is reconciled against official settlement. There is no switching, ladder, or take-profit exit.
...
Selected timing (September 20): **one observed minute → 14-minute forecast**, first P90 + sticky direction, with the five-cent protective exit or authoritative settlement. The default matches this selection.

Prepared account selection: **primary account (subaccount 0)**, as requested. The legacy 100-contract, $100 principal/order, and $100 realized UTC-day loss limits were removed from the v4 configuration. The maximum ask remains $0.99, and every order still requires a sufficient live exchange-shard balance with a fee reserve. There is no daily strategy-loss breaker: losses can accumulate across recovery cycles. Recovery state does not reset with a new runner or UTC day. A new shared-account protocol permits positions from the five separately journaled coin strategies only when an exact terminal exchange fill explains each position. Resting orders, unknown positions, and unresolved intents block new entry; the code never cancels or takes ownership of them. Coin live orders remain disabled while an older pinned BTC worker is still claiming the primary journal.
...
5. **Different account activity:** primary-account selection is supported. The shared-account protocol attributes the other five coin strategies' exact fills through their separate durable journals and serializes submissions. Unexplained positions, resting orders, and ambiguous intent delivery still block new orders. Dedicated subaccounts remain supported if independently selected by the operator.
...
1. Review the implementation and exact strategy/risk configuration. Default account 0 is the primary account; optionally select an existing subaccount 1–63. Key scope/permissions must match that selection. Provision/fund it yourself. This code never opens/funds an account. The five approved coin workers may share account 0 only under the new fenced shared-account protocol; unexplained manual or other-bot positions block new entries.
...
Emergency: set `QUANTURA_KALSHI_CONTINUOUS=false` and `QUANTURA_KALSHI_LIVE_ENABLED=false` to prevent new dispatch/approval. Setting the active private session's `enabled=false` blocks **all** new POSTs, including a protective stop exit; inspect and manage an existing position directly at the exchange if you use that kill switch. Cancel the running Action if necessary. **The kill switch does not liquidate positions or cancel another bot's orders.**

## Remaining production verification

Live fills and settlements have occurred under earlier code, but this does not validate the new stop execution, liquidity at or below five cents, large-order liquidity, partial fills, provisional disagreement rates, or an extended performance claim. A marketable limit cannot guarantee a fill or a five-cent execution price. Offline tests cannot establish these. Hosted CPU model initialization may miss short-horizon opportunities; missed signals are reported rather than backfilled as trades. A persistent worker is preferable if uninterrupted coverage becomes mandatory.
