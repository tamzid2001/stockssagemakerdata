...
1. YouTube (short explainers and demos)
2. Reddit (community feedback and intent traffic)
3. Facebook + Instagram + Threads + TikTok + Pinterest
...
- `SOCIAL_AUTOPILOT_CHANNELS=facebook,instagram,tiktok`
...
- tries direct official API integration first for Facebook/Instagram/TikTok.
...
const draftRes = await gen({
  ...
  channels: ["youtube", "reddit", "facebook", "instagram"],
  postsPerChannel: 2,
...
Health probe (reads/fallback checks across Reddit, Facebook, Instagram, TikTok):
...
```bash
python scripts/sync_github_social_secrets.py --only OPENAI_API_KEY,FACEBOOK_PAGE_ACCESS_TOKEN,INSTAGRAM_ACCESS_TOKEN,TIKTOK_ACCESS_TOKEN
```
