...
- Hold the filled side until authoritative settlement unless its executable economic bid reaches $0.05 or lower. A five-cent stop trigger is durable: while the market is active, the worker checks roughly once per second and submits a reduce-only IOC sell at the current bid, capped at $0.05. It reconciles each order and its actual fills before retrying the unsold quantity with a fresh client order ID. A rejected, unfilled, partially filled, or ambiguous order is never mistaken for a flat position. If the market settles before the stop fully fills, the residual is reconciled against official settlement. There is no switching, ladder, or take-profit exit.
