...
function successExample(operationId: string): unknown {
  ...
  switch (operationId) {
    ...
    case "getForecastResolution": return { data: { forecast_id: forecast.forecast_id, status: "resolved_yes", actual_outcome: "The formal proposition resolved YES.", resolved_at: "2026-11-18T21:15:00.000Z", log_score: 0.4005, resolution_source: "authoritative-source" }, meta: successMeta() };
    ...
    case "getForecastPerformance": return { data: { resolved_forecasts: 125, average_log_score: 0.574, average_probability: 0.66 }, meta: successMeta() };
    ...
    paths: {
      ...
      "/backtests/{id}/strategy": {
        get: { tags: ["Backtests"], operationId: "exportBacktestStrategy", "x-quantura-scope": "backtests:read", parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }],
          description: "Exports a stable JSON strategy plus source, cost model and historical data hash. live_eligible is false: a separately reviewed and authorized execution workflow must validate any live deployment.",
          responses: { "200": { description: "Versioned research configuration for separate live review." }, "404": { description: "Backtest not found." }, ...commonErrors } },
      },
      "/me/access": {
        get: {
      ...
      "/ensemble-forecasts/{forecast_id}/observations": {
        get: {
          ...
          description: "Returns genuine selected-side targets (Kalshi closing ask, Polymarket US display quote) or stock closes after the immutable input cutoff. The first prediction-row minute is retained when available, even outside the most recent 500 observations; worker completion does not move this comparison to another minute. A quote preceding publication is retrospective, not a backdated fill. Stock rows include interval and exchange session_date metadata; minute closes are not final daily closes. Does not run models or revise predictions. Poll no faster than once per minute. Retrieval is bounded to 90 days after input cutoff, subject to provider retention (recent stock minutes use a seven-day window). Dataset-only jobs return no live observations.",
      ...
      "/ensemble-forecasts/{forecast_id}/prepare-save": {
        post: {
          tags:["Ensemble Forecasts"], summary:"Prepare to save a guest forecast after signing in", operationId:"prepareGuestForecastSave", "x-quantura-scope":"forecasts:write",
          parameters:[idParameter], description:"Web guest-session operation. Requires the verified anonymous owner of a completed private forecast. Sets a Secure, HttpOnly, SameSite=Lax cookie valid for 15 minutes. The one-use secret is hashed in storage and is never returned as JSON. After account sign-in, POST the save endpoint using the same browser. Does not grant other workspace access.",
          responses:{"200":{description:"Guest ownership proof prepared; sign in to finish saving.",content:{"application/json":{schema:{type:"object",properties:{data:{type:"object",properties:{forecast_id:{type:"string"},ready_to_sign_in:{type:"boolean"}}}}},example:{data:{forecast_id:"ens_example",ready_to_sign_in:true},meta:{api_version:"v1"}}}}},...commonErrors},
        },
      },
      "/ensemble-forecasts/{forecast_id}/save": {
        post: {
          tags: ["Ensemble Forecasts"], summary: "Save your forecast to your registered account profile", operationId: "saveEnsembleForecastToProfile", "x-quantura-scope": "forecasts:write",
          parameters: [idParameter],
          description: "Requires a registered account, ownership of the forecast and current workspace permissions. Guest sessions can forecast and download, but cannot save to a profile. The website also supports signing in to a different Firebase identity: prepare-save first establishes a 15-minute, single-use HttpOnly ownership cookie. The save operation verifies and consumes that cookie transactionally; neither a forecast ID nor the new login alone grants access.",
          responses: {"200":{description:"Saved to the owner's My Requests profile.",content:{"application/json":{schema:{type:"object",properties:{data:{type:"object",properties:{forecast_id:{type:"string"},saved:{type:"boolean"}}}}},example:{data:{forecast_id:"ens_example",saved:true},meta:{api_version:"v1"}}}}},...commonErrors},
        },
      },
      ...
      "/performance": {
        get: { tags: ["Forecasts"], summary: "Get aggregate forecast performance", operationId: "getForecastPerformance", "x-quantura-scope": "forecasts:resolved (forecasts:read alias supported)", parameters: forecastFilters, responses: { "200": { description: "Logarithmic loss, probability, and calibration aggregates." }, ...commonErrors } },
      },
    ...
    components: {
      ...
      schemas: {
        ...
        BacktestRequest: { type: "object", additionalProperties: false, required: ["source", "forecast", "replay", "strategy"], properties: {
          ...
          ] }, execution: { type: "object", additionalProperties: false, properties: { starting_capital: { type: "number", minimum: 10, maximum: 1000000 }, position_fraction: { type: "number", exclusiveMinimum: 0, maximum: 1 }, commission_bps: { type: "number", minimum: 0, maximum: 1000 }, slippage_bps: { type: "number", minimum: 0, maximum: 1000 } } },
        } },
          ...
          allOf: [
            { $ref: "#/components/schemas/EnsembleConfiguration" },
            {
              type: "object", required: ["source"], properties: {
                ...
                source: {
                  oneOf: [
                    { type: "object", required: ["type", "symbol"], properties: { type: { const: "ticker" }, symbol: { type: "string" }, provider: { type: "string", enum: ["auto", "alpaca", "yahoo"] }, start: { type: "string", format: "date" }, limit: {type:"integer",minimum:2,maximum:10000,default:500,description:"Maximum latest valid observations before cutoff. Model minimum history requirements still apply."}, frequency: {type:"string",enum:["1Day","1Hour","1Min"],default:"1Day"}, field: { type: "string", enum: ["open", "high", "low", "close", "volume"] }, adjustment:{type:"string",enum:["raw","split","dividend","spin-off","all"],default:"raw",description:"Provider price adjustment. Yahoo uses its adjusted-close factor for non-raw requests; it does not distinguish adjustment event types."}, session:{type:"string",enum:["regular","extended"],default:"regular"}, feed:{type:"string",enum:["iex","sip","boats","otc","yahoo"],description:"Alpaca defaults to iex; other feeds require provider entitlement. Yahoo always uses its own feed. The selected basis is persisted for overlays and refreshes."} } },
                    { type: "object", additionalProperties: false, required: ["type", "provider", "symbol", "contract_id"], properties: { type: { const: "prediction_market" }, provider: { type: "string", enum: ["polymarket_us", "kalshi"] }, symbol: { type: "string", description: "Provider market slug (Polymarket US) or market ticker (Kalshi)." }, contract_id: { type: "string", description: "Exact selected side ID from market lookup; verified server-side." }, limit: {type:"integer",minimum:2,maximum:500,default:500,description:"Maximum latest genuine observations after phase, cutoff and time-window filtering. Fewer are used if unavailable; missing bars are never filled. Model minimums apply."}, frequency: { type: "string", enum: ["1min", "1h", "1D"], default: "1min" }, history_phase: {type:"string",enum:["auto","both","pregame","in_game"],default:"both",description:"The website defaults to auto: pregame plus in-game before 32 elapsed game minutes, then in-game only, evaluated at the input cutoff. Missing event start keeps auto on both; explicit phase-specific requests require a verified start. Existing API clients retain both when omitted."}, history_lookback_minutes:{type:"integer",minimum:0,maximum:129600,default:0,description:"0 imposes no extra time restriction; positive values restrict elapsed history before cutoff. For pregame, the window ends at the earlier of cutoff and game start. source.limit then selects the latest N genuine observations."} } },
