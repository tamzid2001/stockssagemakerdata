...
## First-row minute signal

The website compares the **completed one-minute observation at the first predicted timestamp**, immediately after the downloaded input history, with that row's frozen P10/P90 levels. Worker completion time does not change which quote is selected. The price is the same selected-side target used by the history/chart (closing ask for Kalshi, display quote for Polymarket US, closing price for stocks). Below P10 is BUY; above P90 is SELL; otherwise NEUTRAL. Equality is neutral. A missing exact observation remains pending, never replaced by a later quote or forward-filled price. If the forecast finished after this observation, the comparison is explicitly retrospective, not a backdated live entry. Signals are research annotations, not orders, fills or verified strategy returns.
...
## Historical ML metrics at completion

New forecasts include `historical_validation`: MAE, RMSE, sMAPE and average weighted
quantile loss, computed on already-known historical values. No future outcome is
required to display these metrics. The worker withholds the most recent **up to
20% of rows, 30 rows, or the requested horizon, whichever is smallest**, retaining
the minimum context required by every selected model (at least two training rows).
For short histories it can withhold one row; insufficient context is reported explicitly.
It runs the same selected models, weights and quantiles against that holdout with
strict model success. Only earlier rows reach those models; transformations are
derived from the training prefix. Exact timestamps (exchange-session dates for
daily equities) must match. Missing outcomes are never fabricated.

The future forecast still uses **all selected historical inputs**. Validation is a
separate, bounded inference pass in the existing asynchronous worker, so it adds
processing time. Progress identifies the historical-validation phase. Results,
sample sizes, split timestamps, hashes and effective weights are saved with the
forecast; JSON downloads include the public summary. This is a single historical
holdout, **not training-fit accuracy or a multi-window walk-forward backtest**.

`historical_validation.status` is `completed`, `insufficient_history`,
`no_matching_outcomes`, or `failed`. Validation failure does not discard a valid
future forecast or substitute fewer models. Without P50 the point metrics are
`null`; wQL is `null` when all matched actual values are zero. sMAPE is a ratio in
the API (the UI displays a percentage); zero/zero contributes zero.

Older immutable forecasts remain readable. **Reproduce saved configuration**
creates a new job using the saved input/model configuration with historical
validation; the original result is untouched. New jobs use a versioned validation
policy in their cache identity. Existing research collectors and trading workers
do not silently acquire a second inference pass.
