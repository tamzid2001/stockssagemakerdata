...
export function addScreenerOpenapi(document:any):void {
  ...
  const filter={name:"Average P50 comparison",email:false,filters:{quantileRules:[{quantile:"p50",statistic:"avg",operator:"gt",percent:10}]}};
  ...
  document.components.schemas.ScreenerAlertFilters={type:"object",additionalProperties:false,properties:{
    ...
    minMarketCap:{type:["number","null"],minimum:0,default:null},maxMarketCap:{type:["number","null"],minimum:0,default:null},
    positions:{type:"array",default:[],items:{type:"string",enum:["below-p01","above-p01","below-p10","above-p10","below-p50","above-p50","below-p90","above-p90","below-p99","above-p99"]}},
    quantileRules:{type:"array",maxItems:12,default:[],items:{type:"object",additionalProperties:false,required:["quantile","statistic","operator","percent"],properties:{quantile:{type:"string",enum:["p01","p10","p25","p50","p75","p90","p99"]},statistic:{type:"string",enum:["min","max","avg"]},operator:{type:"string",enum:["gt","gte","lt","lte"]},percent:{type:"number",minimum:-100000,maximum:100000,description:"Offset from price in percent: gt/gte add the offset; lt/lte subtract its magnitude. Below by 10 means comparison with 90% of price. Legacy negative below offsets remain equivalent."}}}},
