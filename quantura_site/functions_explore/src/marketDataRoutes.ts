...
import { AlpacaClient, AlpacaError, barsToCsv, publicAlpacaError } from "./alpacaClient";
  ...
  const stockHistory = async (req: any, res: any) => {
    try {
      ...
      if (String(body.format || req.query?.format || "").toLowerCase() === "csv") {
        const filename = `${filePart(result.symbol)}-${provider}-${filePart(result.timeframe)}-${filePart(body.end || range.end)}.csv`;
