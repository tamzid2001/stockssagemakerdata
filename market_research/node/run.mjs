...
const quoteTape = inputs[0] === '--btc-quotes';
if (quoteTape) inputs.shift();
...
async function checkpoint() {
  ...
  try {
    ...
    const prefix = quoteTape ? `btc-quote-tape-${process.env.QUANTURA_CODE_SHA}` : btc ? `btc-paper-checkpoint-${process.env.QUANTURA_CODE_SHA}` : paper ? `p1-paper-checkpoint-${process.env.QUANTURA_CODE_SHA}` : 'replay-checkpoint';
...
const timer = setInterval(() => publish().catch(() => console.error('CHECKPOINT_UPLOAD_FAILED; local checkpoint retained')), quoteTape ? 60 * 1000 : paper || btc ? 5 * 60 * 1000 : 45 * 60 * 1000);
...
try {
  ...
  await command(['-m', quoteTape ? 'market_research.btc_quote_tape' : btc ? 'market_research.kalshi_btc' : paper ? 'market_research.p1_worker' : p1Replay ? 'market_research.p1_historical' : 'market_research.historical', ...inputs]);
}
