...
test('homepage shows no recommendation when a validated quantile row is unavailable',async()=>{
  ...
  assert.match(d.window.document.querySelector('[data-buy-status]').textContent,/No validated stock forecasts/);
...
test('homepage links exact published forecast and displays neutral quantiles',async()=>{
  const d=setup([{ticker:'TEST',company_name:'<img src=x>',actual_price:131,p50:132,p99:136,signal:'buy'}]);await tick();
