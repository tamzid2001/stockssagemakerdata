...
The website compares the **first completed one-minute observation after forecast publication** with the **first predicted row's** frozen P10/P90 levels. These timestamps can differ and both are displayed. Below P10 is a BUY signal for the selected side; above P90 is a SELL signal for that side; otherwise no signal. Equality is not a signal. Missing observations remain pending, never replaced by a later quote or forward-filled price. Historical replay comparisons are labeled separately. Signals are research annotations, not orders, fills or verified strategy returns.
