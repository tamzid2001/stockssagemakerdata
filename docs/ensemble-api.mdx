...
**Data cutoff** defaults to latest data. Choose 15, 30, 60, or a custom number of minutes ago (1–1440) to exclude newer observations **before** selecting up to 500 genuine inputs. The API accepts top-level `history_lag_minutes`; it resolves the cutoff once to a stored UTC minute. For example, use `history_lag_minutes: 30` and `prediction_length: 60` with `1min` data. Forecast timestamps begin after the last actual input, so gaps can shorten the part extending past now.
