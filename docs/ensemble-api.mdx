...
Prediction-market sources accept `history_phase: "both" | "pregame" | "in_game"` (default `both`) and `history_lookback_minutes` (0–129600, default 0). Zero uses up to 500 available observations; 60 restricts the input to the last hour, not 500 fabricated rows. For pregame, lookback ends at the earlier of game start and input cutoff. Game start is resolved from provider metadata, never trusted from client input. Unknown start times reject phase-specific requests.
...
Polymarket pregame and in-game custom timestamp ranges are downloaded separately and joined by original timestamps. Repeated provider display quotes remain in downloads; they are not separate trades. Forecast creation returns `422 HISTORY_FLAT_WINDOW` if the entire selected window is unchanged or at least 30 final observations have the same price over two hours. Changing histories with earlier flat stretches remain usable with a warning. This is a data-quality guard, not proof of predictive skill.

`source.history_quality` records input counts, pregame/in-game counts, price-change count, and longest/trailing unchanged duration. Existing immutable forecasts remain unchanged. The shared backtest and paper-worker inference path applies the same flat-window guard. New results must not be mixed silently with earlier corpus versions.
...
The existing historical export endpoint `/api/sports/prediction-markets/export` also accepts `history_phase` and `history_lookback_minutes`. Its legacy `pregameOnly` parameter remains supported when `history_phase` is absent. Original rows remain downloadable even when their flatness prevents new inference.
