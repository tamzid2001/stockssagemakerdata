(() => {
  ...
  const initCalibration = async () => {
    ...
    try {
      ...
      table.querySelector("tbody").innerHTML = rows.length ? rows.map((row) => `<tr><td>${escapeHtml(row.bucket)}</td><td>${escapeHtml(row.forecast_count)}</td><td>${escapeHtml(formatPercent(row.predicted_average_probability, 1))}</td><td>${escapeHtml(formatPercent(row.actual_event_frequency, 1))}</td></tr>`).join("") : '<tr><td colspan="4">Calibration data will appear after reviewed forecasts resolve.</td></tr>';
    } catch (error) {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    initFeed();
    const calibration = document.getElementById("forecast-performance-summary");
    if (calibration) window.QuanturaUI ? window.QuanturaUI.whenVisible(calibration, initCalibration) : initCalibration();
    initDetail();
    const preview = document.getElementById("home-forecast-preview");
    if (preview) window.QuanturaUI ? window.QuanturaUI.whenVisible(preview, initHomePreview) : initHomePreview();
  });
})();
